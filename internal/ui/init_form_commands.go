...
func runDatabaseForm(db config.ConnectionSettings) (config.ConnectionSettings, bool, error) {
	...
	if initial == nil {
		return config.ConnectionSettings{}, false, fmt.Errorf("edit form cannot seed a %q database", db.Type)
	}

	m, err := tea.NewProgram(newInitFormModel(formSpec, db.Type, initial)).Run()
	if err != nil {
		return config.ConnectionSettings{}, false, err
	}

	fm, ok := m.(initFormModel)
	if !ok {
		return config.ConnectionSettings{}, false, fmt.Errorf("database form returned an unexpected model %T", m)
	}
	if !fm.submitted {
		return config.ConnectionSettings{}, false, nil
	}
